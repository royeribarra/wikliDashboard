...
import { Table, Form, Card as CardAnt, Select, Input, Popover, Button, Tooltip, Image } from "antd";
import { TiendaCambioService } from "../../../servicios/tienda/tiendaCambioService";
...
const Cambios = ({ updateMigas }) => {
  const tiendaCambioService = new TiendaCambioService("tienda/cambios-estandar");
  ...
  let columns = [
    ...
    },
    {
      ...
      render: (tracker_cambio_estandar) => {
        const updateTracker = () => {
          tiendaCambioService.updateStateFromStore(tracker_cambio_estandar.gestion_id, tracker_cambio_estandar).then((response) => {
            fetchAll();
          });
        }

        const aceptarCambio = () => {
          tiendaCambioService.aceptarCambio(tracker_cambio_estandar.gestion_id, tracker_cambio_estandar.id).then((response) => {
            fetchAll();
          });
        }

        const denegarDevolucion = (values) => {
          tiendaCambioService.denied(tracker_cambio_estandar.gestion_id, values).then((response) => {
            fetchAll();
          });
        }
          ...
          <>
            {
              ...
              <>
                {tracker_cambio_estandar.estado===1 && 
                <Fragment>
                  <p>Operador logístico por confirmar (RECOJO).</p>
                </Fragment>
                }
                {tracker_cambio_estandar.estado===2 && 
                  <Fragment>
                    <p>Operador logístico en camino a "RECOJO".</p>
                  </Fragment>
                }
                {tracker_cambio_estandar.estado===3 && 
                  <Fragment>
                ...
                {tracker_cambio_estandar.estado===6 && 
                  <Fragment>
                    <p>Operador logístico por confirmar (ENTREGA).</p>
                  </Fragment>
                }
                {tracker_cambio_estandar.estado===7 && 
                  <Fragment>
                    <p>Producto nuevo en camino</p>
                  </Fragment>
                }
                {tracker_cambio_estandar.estado===8 && 
                  <Fragment>
                    <p>Producto nuevo entregado</p>
                  </Fragment>
                }
                {tracker_cambio_estandar.estado===9 && 
                  <Fragment>
                    <p>Finalizado</p>
                  </Fragment>
                }
  ...
  let columnsError = [
    ...
    },
    {
      ...
      render: (id) => {
        const Aceptar = () => {
          tiendaCambioService.aceptarCambioErrorTienda(id).then((data) => {
            fetchAll();
            fetchAllByError();
          })
        }
        const Denegar = () => {
          tiendaCambioService.denegarCambioErrorTienda(id).then((data) => {
            fetchAll();
            fetchAllByError();
          })
        }
  ...
  const fetchAll = (paginationTab = pagination) => {
    ...
    tiendaCambioService.getAll(searchObj).then(({ data }) => {
      setPagination({
        ...paginationTab,
      ...
      <Buscar form={form} handleParentSearch={fetchAll}/>
      {
        ...
        <Card style={{marginBottom: "15px"}}>
          <CardHeader style={{color: "red"}}>
            Lista de cambios Estándar (por aprobar)
          </CardHeader>
          <CardBody>
            <Table
...
export default connect(mapStateToProps, mapDispatchToProps)(Cambios);